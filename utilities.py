...
def getGrid (Lx, Ly, unit_distance,dstops, n, p_res, rhor_y, rvert_x, r3x, r3y):
    ...
    ### Depot
    x_depot = x_range[int(len(x_range)/2)]
    y_depot = y_range[int(len(y_range)/2)]     
    for i in nodes:
    ...
    y_31_dif  = {y: abs(y - y31_init) for y in y_range}
    y31 = min (y_31_dif, key = y_31_dif.get) # the y that is closest
    
    y32_init = int(r3y[1]*max_len_y)
    y32_diff  = {y: abs(y-y32_init) for y in y_range}
    y32 = min (y32_diff, key = y32_diff.get) # the y that is closest
...
def transformInput (nodes, dx, dy, twindows,demand,twlen,tw_start_end,
    ...
    for node in nodes:
        ...
        for tw in twindows:
            ...
            tw_start_tr_node[trnode] = tw_start_end[tw][0] - offset 
