...
class CaseStudy:
    def __init__ (self, network, demav, peak_to_off, route_costs, stop_costs, c_tm, c_dwell, c_local, c_late, c_freq,
        ...
        self.end = end
        self.tw_len = tw_len
        self.bus_start = bus_start # bus_start = 9
        self.bus_end = bus_end # bus_end = 17
        ...
        (self.c_route, self.c_stop, self.vehicle_route, self.b_freq_peak_route, self.b_freq_off_route) = self.F_RANDOMIES()
...
#                print ('BE CAREFUL! NODE', demnode, 'UNSERVED DUE TO DISTANCE!')
    ...
    def getDemand (self):
        ...
        if one_tw_per_node == 0: # If the demand of a node is spread across multiple time windows
...
#                    print ('AGAMHSOU', area_nodes, areadem)
                    dictt[tw] = utilities.getSum (area_nodes, areadem)
    #            print ('kipi',dictt)
                for tw in dictt:
                    for node in dictt[tw]:
                        demand[node,tw] = dictt[tw][node]
        ...
        for i in routes:
            for t in twindows:
                if t in twindows_peak:
                    b_freq_tw[i,t] = b_freq_peak_route[i] * tw_len # fpeak in buses per hour, tw_len in hours/time window
                else:
...
#    def getVmtBb (self,f_meters_to_miles):
#        routes = self.network.routes
#        twindows = self.twindows
#        length_route = self.network.length_route
#        b_freq_tw = self.b_freq_tw
...
#            passes = 0
#            for tw in twindows:
#                passes = passes + b_freq_tw[route,tw]
#            vmt_basic_route[route] = passes*len_total*f_meters_to_miles
#        vmt_bus_basic_total = sum(vmt_basic_route.values())
#        return (vmt_basic_route, vmt_bus_basic_total)
#    
...
class Network:
    ...
    def getDist (self):
        ...
        for node1 in nodes:
            for node2 in nodes:
#                dist[node1,node2] = round(((X[node1]-X[node2])**2+(Y[node1]-Y[node2])**2)**0.5)
