...
class CreateDemandCallback(object):
  ...
  def Demand(self, from_node, to_node):
    return -self.matrix[from_node]  #################### Changed this!!!
...
def main(data, num_vehicles, time_per_demand_unit, horizon, speed, VehicleCapacity, first_reload_node_index) :  #################
#  print ('Now in main VRPTW')
  RES = [] 
  # Create the data.
#  data = create_data_array(data) ## added "data"
  locations = data[0]
  demands = data[1]
  start_times = data[2]
  end_times = data[3]
  ...
  depot = 0
#  num_vehicles = 8
#  search_time_limit = 1200
  
  # Create routing model.
  if num_locations > 0:
#    print ('num_locations>0')
...
#    print ('Now finding distances')
    dist_between_locations = CreateDistanceCallback(locations)
    dist_callback = dist_between_locations.Distance

    routing.SetArcCostEvaluatorOfAllVehicles(dist_callback)
#    print ('Now finding demands')
    demands_at_locations = CreateDemandCallback(demands)
    demands_callback = demands_at_locations.Demand

    # Adding capacity dimension constraints.
#    print ('Now at "adding capacity dimension constraints"')
#    VehicleCapacity = 150;
#    NullCapacitySlack = 0;
...
#                         fix_start_cumul_to_zero, capacity)
    ...
    routing.AddDimension(demands_callback, VehicleCapacity, VehicleCapacity,  ############ Changed this!!!
                         fix_start_cumul_to_zero, capacity)
    
    
    capacity_dimension = routing.GetDimensionOrDie(capacity)  ###### Changed this!!!
    for order in range(1,num_locations):
        if  order < first_reload_node_index: #  --- > Changed to if not reloading, 16/7 #######
            capacity_dimension.SlackVar(order).SetValue(0)
        routing.AddVariableMaximizedByFinalizer(capacity_dimension.CumulVar(order))


    # Add time dimension.
#    print ('Now at "add time dimension"')
...
#    print ('Now adding time window constraints')
    time_dimension = routing.GetDimensionOrDie(time)
#    print ('after time dimension')
#    for location in range(1, num_locations):
    for location in range(2, first_reload_node_index): ### Without depot and re-loading (i.e. depot), 16/7 ########################
    ...
    penalty = 10000000
    for order in range(1, first_reload_node_index):  ### Without depot and re-loading (i.e. depot) ########################
...
#  locations = [[820, 760], [960, 440],[960, 440], [500, 50], [490, 80], [130, 70], [290, 890], [580, 300],
...
#
#  demands =  [0, 19, 21, 22, 6, 19, 7, 12, 16, 6, 16, 8, 14, 21, 16, 3, 22, 18,
#             19, 1, 24, 8, 12, 4, 8, 24, 24, 2, 20, 15, 2, 14, 9]
#
#  start_times =  [0, 508, 444, 103, 493, 225, 531, 89,
